...
while True:
    ...
    for cnts in contours:
        ...
        if M['m00'] != 0 and cv2.contourArea(cnts) > 1000:
            ...
            BD.append([counterframe,counterobj,idDisp,cX,cY])

            print('BD: ')
            print(busca(BD, 1,counterframe))

            if counterframe > 1:
                
                quadroanterior.extend(busca(BD, 1,counterframe-1))

                print(quadroanterior)

                for i in range(len(quadroanterior)):

                    cXa = int(quadroanterior[i][3])
                    cYa = int(quadroanterior[i][4])
            ...
            escrever(grayr, 'Pc {}'.format(idPeca) , cX, cY-45)
